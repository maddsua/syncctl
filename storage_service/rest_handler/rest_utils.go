...
func (byterange *ByteRange) IsZero() bool {
	return byterange.Start <= 0 && byterange.End <= 0
}

func (byterange *ByteRange) actualEnd() int64 {

	if byterange.End > 0 {
		return byterange.End
	}

	return byterange.TotalSize
}

func (byterange *ByteRange) Size() int64 {
	return byterange.actualEnd() - byterange.Start
}

func (byterange *ByteRange) String() string {
	return fmt.Sprintf("bytes %d-%d/%d", byterange.Start, byterange.actualEnd(), byterange.TotalSize)
}

func (byterange *ByteRange) Parse(val string) error {
